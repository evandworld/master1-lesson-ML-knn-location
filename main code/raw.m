...
for i=1:size(p_d)   %按顺序分别给每一个数据定位
    [size_x, size_y, n_AP] = size(fingerprint);
    %计算欧氏距离
    distance = 0;
    for j=1:n
        distance = distance + (fingerprint(:,:,j)-RSS_d(i,j)).^2;%这里同时计算所有参考点，结果是一个二维矩阵
    end
...
plot(p_d(:,1),p_d(:,2),'bo');  %绘制测试集真实位置
axis([0 7 0 4])
grid on;hold on;
plot(p_KNN(:,1),p_KNN(:,2),'r*');  %绘制测试集的计算位置

for i=1:size(p_KNN)
    hold on;plot([p_KNN(i,1),p_d(i,1)],[p_KNN(i,2),p_d(i,2)],'g--');  %绘制偏差，便于观察
end
legend('真实位置','计算位置','偏差')
error_KNN=sqrt((p_d(:,1)-p_KNN(:,1)).^2+(p_d(:,2)-p_KNN(:,2)).^2);
disp('KNN平均误差：')
disp(mean(error_KNN));
